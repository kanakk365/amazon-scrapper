...
export default function Navbar() {
  ...
  return (
    <section id="navbar" className="fixed w-full top-0 z-50 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-4 bg-transparent mt-1 border-2 border-gray-200 rounded-lg">
        ...
        {mobileMenuOpen && (
          <div id="mobile-menu" className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <MobileNavLink to="#howItWorks" onClick={() => setMobileMenuOpen(false)}>
                How It Works
              </MobileNavLink>
...
function NavLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link to={to} className="text-[#1d293d] hover:text-black hover:scale-110 duration-100 transition-colors">
      {children}
    </Link>
  )
}
...
function MobileNavLink({
  ...
  return (
    <Link to={to} className="block px-3 py-2 text-black transition-colors" onClick={onClick}>
      {children}
    </Link>
  )
}
